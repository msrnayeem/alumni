<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <!-- Degree -->
        <div class="mt-4">
            <x-input-label for="degree" :value="__('Degree (e.g. BSc, MSc)')" />
            <x-text-input id="degree" class="block mt-1 w-full" type="text" name="degree" :value="old('degree')" required />
            <x-input-error :messages="$errors->get('degree')" class="mt-2" />
        </div>

        <!-- Major -->
        <div class="mt-4">
            <x-input-label for="major" :value="__('Major/Department')" />
            <x-text-input id="major" class="block mt-1 w-full" type="text" name="major" :value="old('major')" required />
